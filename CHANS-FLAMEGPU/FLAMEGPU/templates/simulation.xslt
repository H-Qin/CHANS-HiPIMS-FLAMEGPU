...
<xsl:if test="xmml:name ='FLOOD_DATA_ARRAY' ">
void init_flood_data_array(){
	if(FLOOD_DATA_ARRAY == nullptr){
		cudaMalloc((void **)&amp;FLOOD_DATA_ARRAY, <xsl:value-of select="xmml:arrayLength"/> * sizeof(<xsl:value-of select="xmml:type"/>));
...
//constant setter
void set_<xsl:value-of select="xmml:name"/>(<xsl:value-of select="xmml:type"/>* h_<xsl:value-of select="xmml:name"/>){
<xsl:variable name="v_name" select ="xmml:name"/>
<xsl:choose>
	<xsl:when test="$v_name = 'FLOOD_DATA_ARRAY'">
	
	init_flood_data_array();
	
	gpuErrchk(cudaMemcpy(FLOOD_DATA_ARRAY, h_FLOOD_DATA_ARRAY, <xsl:value-of select="xmml:arrayLength"/> * sizeof(<xsl:value-of select="xmml:type"/>), cudaMemcpyHostToDevice));
...
void <xsl:value-of select="../../xmml:name"/>_<xsl:value-of select="xmml:name"/>(cudaStream_t &amp;stream){
  ...
  <xsl:if test="gpu:partitioningNone or gpu:partitioningSpatial or gpu:partitioningGraphEdge">//Set the message_type for non partitioned, spatially partitioned and On-Graph Partitioned message outputs
	...
	GPUFLAME_<xsl:value-of select="xmml:name"/>&lt;&lt;&lt;g, b, sm_size, stream&gt;&gt;&gt;(d_<xsl:value-of select="../../xmml:name"/>s<xsl:if test="xmml:xagentOutputs/gpu:xagentOutput">, d_<xsl:value-of select="xmml:xagentOutputs/gpu:xagentOutput/xmml:xagentName"/>s_new</xsl:if>
		...
		<xsl:if test="gpu:RNG='true'">, d_rand48</xsl:if>  <xsl:if test="xmml:name = 'update_data'">, FLOOD_DATA_ARRAY</xsl:if>);
	gpuErrchkLaunch();
...
<xsl:if test="gpu:partitioningGraphEdge">
  ...
  gpuErrchkLaunch();
	...
	//printf("<xsl:value-of select="../../xmml:name"/>_<xsl:value-of select="xmml:name"/> --- %d and %d --- %p --- %p \n", d_xmachine_memory_navmap_static_count, h_xmachine_memory_navmap_static_count, &amp;d_xmachine_memory_<xsl:value-of select="../../xmml:name"/>_<xsl:value-of select="xmml:currentState"/>_count, &amp;h_xmachine_memory_<xsl:value-of select="../../xmml:name"/>_<xsl:value-of select="xmml:currentState"/>_count);

	gpuErrchk( cudaMemcpyToSymbol( d_xmachine_memory_<xsl:value-of select="../../xmml:name"/>_<xsl:value-of select="xmml:currentState"/>_count, &amp;h_xmachine_memory_<xsl:value-of select="../../xmml:name"/>_<xsl:value-of select="xmml:currentState"/>_count, sizeof(int)));
	
	//printf("<xsl:value-of select="../../xmml:name"/>_<xsl:value-of select="xmml:name"/> gpuErrchk finished \n");

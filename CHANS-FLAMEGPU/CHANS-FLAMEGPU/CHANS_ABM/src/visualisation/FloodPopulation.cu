...
//KERNEL DEFINITIONS
/** output_floods_to_TBO
 * Outputs flood agent data from FLAME GPU to a 4 component vector used for instancing
 * @param	agents	flood agent list from FLAME GPU
 * @param	data four component vector used to output instance data 
 */
__global__ void output_floods_to_TBO(xmachine_memory_flood_list* agents, glm::vec4* data){
	...
	data[index].z = angle;
	data[index].w = agents->flood_h[index];
}
...
extern void generate_instances2(GLuint* instances_tbo, cudaGraphicsResource_t * instances_cgr)
{
	...
	if (get_agent_flood_static_count() > 0)
	{
		...
		//cuda block size
		tile_size = (int) ceil((float)get_agent_flood_static_count()/threads_per_tile);
		grid = dim3(tile_size, 1, 1);
		threads = dim3(threads_per_tile, 1, 1);
		//call kernel
		output_floods_to_TBO<<< grid, threads>>>(get_device_flood_static_agents(), dptr_1);
		gpuErrchkLaunch();
		// unmap buffer object
...
void displayMapNumber2(int map_no){
	h_currentMap2 = map_no;
	gpuErrchk(cudaMemcpyToSymbol( currentMap, &map_no, sizeof(int)));	
}

int getCurrentMap2()
{
	return h_currentMap2;
}

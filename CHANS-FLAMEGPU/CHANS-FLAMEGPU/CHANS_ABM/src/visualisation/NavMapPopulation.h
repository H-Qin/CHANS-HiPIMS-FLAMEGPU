...
{  
	...
	"   bool is_force = true;														\n"
	"   bool is_DEM = false;														\n"
	"   if (angle > 7.0f && angle < 9.0f){	//more than 360 degrees(6.28 rads)		\n"
	"       is_force = false;														\n"
	"		angle = 1.57079633f;													\n"
	"	}																			\n"
	"   if (angle > 9.0f){	//meaning DEM											\n"
	"       is_force = false;														\n"
	"       is_DEM = true;															\n"
	"		height = angle - 10.0;													\n"
	"		angle = 1.57079633f;													\n"
	...
	"	}else if (flood_h > 0.1f){													\n"
	"		gl_FrontColor = vec4(0.2, 0.0, 1, 1);    								\n"
	"	}else if (flood_h > 0.01f){													\n"
	"		gl_FrontColor = vec4(0.3, 0.7, 0.7, 1);    								\n"
	"	}else if (flood_h > 0.001f){												\n"
	"		gl_FrontColor = vec4(0.3, 1, 1, 0.5);	   								\n"
	"	}else{																		\n"
	...
	"	if (is_DEM) {																\n"
	"		position.z += height * 0.3125 * 0.0775;									\n"
	"	}else{																		\n"
	"		position.z += instance.w * 0.03 *0.0775;								\n"
	"	}    																		\n"

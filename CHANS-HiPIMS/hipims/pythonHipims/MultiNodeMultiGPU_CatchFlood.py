...
def run(rank, size, paraDict):
    ...
    if rank == 0:
        ...
        dt_list = []
        t_list = []
    dem_main, cell_type_mask_main, demMeta = importDEMData_And_BC(
    ...
    dist.all_reduce_multigpu(numerical.get_h_overlap(),
                             op=torch.distributed.ReduceOp.SUM,
                             async_op=False)
    dist.all_reduce_multigpu(numerical.get_wl_overlap(),
                             op=torch.distributed.ReduceOp.SUM,
                             async_op=False)
    dist.all_reduce_multigpu(numerical.get_z_overlap(),
                             op=torch.distributed.ReduceOp.SUM,
                             async_op=False)
    dist.all_reduce_multigpu(numerical.get_qx_overlap(),
                             op=torch.distributed.ReduceOp.SUM,
                             async_op=False)
    dist.all_reduce_multigpu(numerical.get_qy_overlap(),
                             op=torch.distributed.ReduceOp.SUM,
                             async_op=False)
    while numerical.t.item() < paraDict['EndTime']:
        ...
        dist.all_reduce_multigpu(numerical.dt,
...
def init_processes(fn, paraDict, backend='nccl'):
    ...
    # rank should consider the world size
